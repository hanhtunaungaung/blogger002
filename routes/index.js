...
router.get('/', function(req, res, next) {
  Post.find({}).sort({like: -1}).limit(5).populate("author", "name").exec(function(err, rtn){
    if(err)throw err;
...
router.get("/postdetail/:pid", function(req, res){
  Post.findById(req.params.pid).populate("author", "name").exec(function(err, rtn){
    if (err) throw err;
    ...
    .exec(function(err2,rtn2){
      ...
      if(req.session.user){
        ...
        reactStatus = [];
        favStatus = [];
        res.render("postdetail", {post : rtn, comments : rtn2, reactStatus : reactStatus, favStatus : favStatus});
      }
   });
  });
});
